...
## Phase 2 — Target Suggestions

Based on the scan, suggest 5-8 optimization targets specific to THIS repo. Each target should be a concrete aspect of the codebase that can be measured with binary yes/no evals.

### How to generate suggestions

Draw from what you found in Phase 1. Suggestions should be relevant, not generic. Examples of the breadth possible:

- A Python API repo might get: "error handling patterns", "test case quality", "docstring completeness", "type annotation coverage", "API response consistency"
- A React frontend might get: "accessibility compliance", "component prop documentation", "CSS-in-JS naming", "error boundary coverage", "bundle import patterns"
- A technical docs repo might get: "SEO meta tags", "heading hierarchy", "code example correctness", "link validity", "terminology consistency", "readability level"
- A SQL/database repo might get: "query performance patterns", "index usage", "naming conventions", "migration safety", "constraint coverage"
- A CLI tool might get: "help text quality", "argument validation", "error message clarity", "exit code correctness"
- A DevOps/IaC repo might get: "Dockerfile layer efficiency", "secret handling", "resource limit definitions", "health check coverage"

These are examples, not limits. Any measurable aspect of any repo is a valid target.

### Output to the user

Present the suggestions as a numbered list. Add:

> Pick a number, or describe your own optimization goal in your own words.

If the user provides their own goal (either here or in their original message), skip suggestions and go directly to Phase 3 with that goal.
...
1. **`prompt.txt`** — the initial prompt/instructions you will use to generate outputs for the chosen target. Write a reasonable first-draft prompt based on your understanding of the target and the repo's existing conventions.
...
17. If the user provides their optimization goal upfront in their message, skip Phase 2 (suggestions) and go directly to Phase 3 (metrics) with their stated goal.
